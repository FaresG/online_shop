...
<div class="basis-1 flex py-7 px-3 border-b border-solid border-b-gray-200 gap-3">
    <div class="basis-1/5">
        <img src="{{ $cartItem->article->thumbnail }}" alt="">
    </div>
    <div class="basis-3/5">
        <div class="flex flex-col gap-3">
            <a href="{{ route('articles.show', ['article' => $cartItem->article]) }}"><h1 class="text-xl hover:underline">{{ $cartItem->article->title }}</h1></a>
            <p class="text-xs">{{ $cartItem->article->description }}</p>
    ...
    <div class="basis-1/5 flex flex-col text-right justify-between">
        <div>
            ...
            <p id="price-{{ $cartItem->id }}" class="text-xl font-medium" data-value="{{ $cartItem->article->price }}">${{ number_format($cartItem->article->price * $cartItem->quantity, 2) }}</p>
        </div>
        <div>
